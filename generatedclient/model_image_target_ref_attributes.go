/*
Snyk API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: REST
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the ImageTargetRefAttributes type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ImageTargetRefAttributes{}

// ImageTargetRefAttributes struct for ImageTargetRefAttributes
type ImageTargetRefAttributes struct {
	Platform *Platform `json:"platform,omitempty"`
	TargetId *string `json:"target_id,omitempty"`
	TargetReference *string `json:"target_reference,omitempty"`
}

// NewImageTargetRefAttributes instantiates a new ImageTargetRefAttributes object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewImageTargetRefAttributes() *ImageTargetRefAttributes {
	this := ImageTargetRefAttributes{}
	return &this
}

// NewImageTargetRefAttributesWithDefaults instantiates a new ImageTargetRefAttributes object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewImageTargetRefAttributesWithDefaults() *ImageTargetRefAttributes {
	this := ImageTargetRefAttributes{}
	return &this
}

// GetPlatform returns the Platform field value if set, zero value otherwise.
func (o *ImageTargetRefAttributes) GetPlatform() Platform {
	if o == nil || IsNil(o.Platform) {
		var ret Platform
		return ret
	}
	return *o.Platform
}

// GetPlatformOk returns a tuple with the Platform field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ImageTargetRefAttributes) GetPlatformOk() (*Platform, bool) {
	if o == nil || IsNil(o.Platform) {
		return nil, false
	}
	return o.Platform, true
}

// HasPlatform returns a boolean if a field has been set.
func (o *ImageTargetRefAttributes) HasPlatform() bool {
	if o != nil && !IsNil(o.Platform) {
		return true
	}

	return false
}

// SetPlatform gets a reference to the given Platform and assigns it to the Platform field.
func (o *ImageTargetRefAttributes) SetPlatform(v Platform) {
	o.Platform = &v
}

// GetTargetId returns the TargetId field value if set, zero value otherwise.
func (o *ImageTargetRefAttributes) GetTargetId() string {
	if o == nil || IsNil(o.TargetId) {
		var ret string
		return ret
	}
	return *o.TargetId
}

// GetTargetIdOk returns a tuple with the TargetId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ImageTargetRefAttributes) GetTargetIdOk() (*string, bool) {
	if o == nil || IsNil(o.TargetId) {
		return nil, false
	}
	return o.TargetId, true
}

// HasTargetId returns a boolean if a field has been set.
func (o *ImageTargetRefAttributes) HasTargetId() bool {
	if o != nil && !IsNil(o.TargetId) {
		return true
	}

	return false
}

// SetTargetId gets a reference to the given string and assigns it to the TargetId field.
func (o *ImageTargetRefAttributes) SetTargetId(v string) {
	o.TargetId = &v
}

// GetTargetReference returns the TargetReference field value if set, zero value otherwise.
func (o *ImageTargetRefAttributes) GetTargetReference() string {
	if o == nil || IsNil(o.TargetReference) {
		var ret string
		return ret
	}
	return *o.TargetReference
}

// GetTargetReferenceOk returns a tuple with the TargetReference field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ImageTargetRefAttributes) GetTargetReferenceOk() (*string, bool) {
	if o == nil || IsNil(o.TargetReference) {
		return nil, false
	}
	return o.TargetReference, true
}

// HasTargetReference returns a boolean if a field has been set.
func (o *ImageTargetRefAttributes) HasTargetReference() bool {
	if o != nil && !IsNil(o.TargetReference) {
		return true
	}

	return false
}

// SetTargetReference gets a reference to the given string and assigns it to the TargetReference field.
func (o *ImageTargetRefAttributes) SetTargetReference(v string) {
	o.TargetReference = &v
}

func (o ImageTargetRefAttributes) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ImageTargetRefAttributes) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Platform) {
		toSerialize["platform"] = o.Platform
	}
	if !IsNil(o.TargetId) {
		toSerialize["target_id"] = o.TargetId
	}
	if !IsNil(o.TargetReference) {
		toSerialize["target_reference"] = o.TargetReference
	}
	return toSerialize, nil
}

type NullableImageTargetRefAttributes struct {
	value *ImageTargetRefAttributes
	isSet bool
}

func (v NullableImageTargetRefAttributes) Get() *ImageTargetRefAttributes {
	return v.value
}

func (v *NullableImageTargetRefAttributes) Set(val *ImageTargetRefAttributes) {
	v.value = val
	v.isSet = true
}

func (v NullableImageTargetRefAttributes) IsSet() bool {
	return v.isSet
}

func (v *NullableImageTargetRefAttributes) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableImageTargetRefAttributes(val *ImageTargetRefAttributes) *NullableImageTargetRefAttributes {
	return &NullableImageTargetRefAttributes{value: val, isSet: true}
}

func (v NullableImageTargetRefAttributes) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableImageTargetRefAttributes) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


